...
* **Take detailed notes in the incident manager document.** No action or discovery is too small to note. Write down current hypotheses, link to dashboards that have evidence, add relevant customer reports, etc.
    > Example
    > _9:34am_ - Emily found a metric for failing requests to a 3rd party that coincides with the reports our customers are telling us. Current hypothesis is that our logging in functionality has a single point of failure with the 3rd party.
